    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-4 md:grid-cols-2">
      ...
      <div class="bg-white rounded-lg shadow-md dark:bg-gray-800" wire:key="brand-{{ $brand->id }}">
        <a href="/products?selected_brands[0] ={{ $brand->id }}" class="">
          <img src="{{ $brand->image ? url('storage', $brand->image) : asset('images/placeholder.png') }}" alt="{{ $brand->name }}" class="object-cover w-full h-64 rounded-t-lg">
        </a>
...
<div class="bg-orange-200 py-20">
  ...
  @foreach ($categories as $category)
      <a class="group flex flex-col bg-white border shadow-sm rounded-xl hover:shadow-md transition dark:bg-slate-900 dark:border-gray-800 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600" href="/products?selected_categories[0]={{ $category->id }}" wire:key="category-{{ $category->id }}">
